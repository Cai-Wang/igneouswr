...
**根因：** 分类图底图分区标注（"Gabbro"、"metaluminous"等）历史上用硬编码 fontsize，不受 rcParams 控制。裸图 203mm 宽正常，缩小后大了 2.7 倍。

**v2.3 修复：** `_style.base_fs(ax)` 返回 `ax_width_mm / 203.0`（下限 0.25）。所有 `ax.text(fontsize=N)` 改为 `N * base_fs(ax)`。裸图不变，小画布自动缩小。四个 diagram 文件共 49 处改动。调用方无需传额外参数——传 ax 即可自动适配。
