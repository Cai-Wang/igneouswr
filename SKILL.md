...
2. ✅ **Batch 2 (done)**: 搬注册表 `DiagramSpec` + `DIAGRAM_REGISTRY` 到 `diagrams/registry.py`。DiagramSpec 新增 3 个校正字段（review_status/source_ref/review_note），默认值不破旧 6 参写法。修复了旧 import 来源与函数实际所在模块不一���的历史问题（plot_co_th、plot_an_ab_or 等 10+ 个函数）。
