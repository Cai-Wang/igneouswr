...
**时序约束：Tick 对象级样式必须在 finalize() 之后设置。**

**时序铁律：** `plot_*` → `finalize` → `auto_xlim_padding` → `apply_*_axis_style` → `save`。**

| 谁 | 管什么 | 禁止做什么 | 调用时机 |
|----|--------|----------|----------|
| **IgneousWR `plot_*`** | 轴标签文本、边界线/多边形、标准化数据、轴范围/尺度、参考线、数据分组、刻度位置/值、画线设 `label=` | 禁止设 `fontsize=`；禁止调 `tick_params`；禁止画图例；禁止设 Tick 对象级属性 | 内容阶段（任意顺序） |
| **figkit `finalize`** | 子图位置重排、auto_gap 间距测量 | 不能设刻度风格、字体 | finalize 之后不再调 set_position() |
| **调用方后置函数**（出图脚本中） | REE/Spider X轴刻度交替内外、标签偏移旋转、Y轴竖排、xlim padding | 必须在 finalize 之后调用，必须在 save 之前调用 | **finalize 之后，save 之前** |
| **DEPRECATED: apply_format/apply_style** | 不再调用 | — | — |

**注意：`apply_spider_axis_style`、`apply_ree_axis_style`、`auto_xlim_padding` 当前不在 IgneousWR 库中。** 它们在 SKILL.md 文档里描述了但代码尚未实现。出图脚本中需自行实现这些后置样式逻辑（刻度交替、标签旋转、xlim 自适应 padding），放在 finalize 之后 save 之前调用。
