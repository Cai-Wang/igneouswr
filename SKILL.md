...
**新工作项（2026-06-10 用户提出）：坐标数据抽离。**
将 `_classification.py`、`_xy_diagrams.py`、`_tectonic.py` 中内联的分界线坐标数组（`np.array([...])` 块）抽离到 `data/` 目录下的独立 JSON 文件。绘图函数改为启动时 `json.load()` 加载。目标：`_classification.py` 从 1423 行 → ~500 行（纯绘图逻辑），`_xy_diagrams.py` 从 1159 行 → ~600 行。每个图件的坐标文件命名为 `{prefix}_{nn}_{shortname}.json` 如 `cls_02_k2o_sio2.json`。**注意**：此工作不应与校正并行——先校正定稿再抽离，或先抽离再在校正中直接改 JSON 文件。两种顺序均可，但不能混合进行（会导致同一批修改分散在 .py 和 .json 中难以追溯）。
...
- ❌ **分界线坐标与画图逻辑混在同一个文件中的后果**：`_classification.py` 是 1423 行，`_xy_diagrams.py` 是 1159 行，其中 ~40-60% 的字符属于坐标数组而非逻辑代码。用户明确表示"太长了，不敢改"。这会导致维护退缩——每次想改一条分界线都要在 1400 行里搜索+判断。**正确做法**：分界线坐标（无论是三元图 3 维还是 XY 图 2 维）应抽离到 `references/` 或新 `data/` 目录下的独立 JSON/YAML 文件中，绘图函数只做 `json.load()` 加载+渲染。特别是 XY 图的坐标（如 TAS 分类边界、Whalen 判别线、Debon 标尺网格）目前全部内联在函数体中，应优先外置。这个分离目前还**没有做**——它不同于已经完成的 Batch 1/2/3，是一个新工作项。
