...
- ✅ **Git 版本控制**：`git init` 后在 `/home/twoper/.hermes/skills/data-science/igneous-geochemistry/` 下管理。`.gitignore` 排除 `__pycache__/`、`*.png`、`whole_rock_output/`。每次功能性修改后 `git commit`（目前 2 commits: v1.0 + docstring fix）。执行 `git log --oneline` 确认最新状态。
