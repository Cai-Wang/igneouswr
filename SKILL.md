...
├── quick_validate.py       # 验证脚本（--quick 模式跳过出图，秒级回归；含 preflight 依赖版本检查）
...
**中期待办（arch 拆包）：**
`scripts/` → `scripts/igneous_geochem/`，拆分 `io/`（Excel读取/schema校验）、`core/`（GeochemData/标准化/岩性）、`diagrams/`（registry.py + 各模块）、`report/`、`batch/`、`cli.py`
