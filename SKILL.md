...
架构迁移已完成。全部活跃代码在 `igneous_wr/` 包中。旧包 `whole_rock/` 已删除。

- `core/chem.py` ← `_chem.py`
- `core/normalize.py` ← `_normalize.py`
- `core/ternary.py` ← `_ternary.py`
- `report/style.py` ← `_style.py` 旧文件通过 `sys.modules` 接管门面保持旧入口兼容。每次修改后确认 `quick_validate.py --quick` 通过。

**Batch 4（坐标外置）完成**：坐标数组已外置到 `boundaries/{category}/{name}.json`。`_xy_diagrams.py` 已拆散。
...
**实现机制：** 每个绘图函数内部硬编码调用 `_style.save_fig(fig, '短名.png', out_dir)`，但 `save_fig` 通过 `_SHORT_TO_LONG` 映射表（定义在 `igneous_wr/report/style.py` 顶部）自动将短名替换为带编号的长名保存。新增图件时必须在该映射表中添加对应条目，否则输出文件会缺少编号前缀。`FILENAME_MAP` 在 `registry.py` 中已正确注册所有长名，但实际保存由 `_SHORT_TO_LONG` 控制——两者必须保持同步。
...
| CLS-28 | Middlemost1991_Plutonic.png | Middlemost Na₂O+K₂O–SiO₂ 深成岩分类 | SiO₂, Na₂O, K₂O |\n| CLS-29 | Pearce1996_NbY_ZrTi.png | Pearce (1996) Zr/Ti–Nb/Y 火山岩分类图（GCDkit 精确坐标，log-log，10 区域彩色填充） | Zr, Ti, Nb, Y |
...
| SRC-03 | Pearce2008_ThYb_NbYb.png | Pearce Th/Yb–Nb/Yb 源区判别（已升级：灰色MORB-OIB带状阵列+三参考点+弧区标注） | Th, Nb, Yb |
...
├── _style.py               # 样式/字体/配色/投点/保存/HTML报告（sys.modules 门面 → igneous_wr.report.style）
├── _normalize.py           # 标准化参考值常量（sys.modules 门面 → igneous_wr.core.normalize）
├── _chem.py                # feot_calc 等化学计算（sys.modules 门面 → igneous_wr.core.chem）
├── _ternary.py             # 三元图坐标变换（sys.modules 门面 → igneous_wr.core.ternary）
...
├── igneous_wr/        # 当前活跃目录
    ├── boundaries/          # 坐标边界数据（JSON）。仅 cls/ 和 tec/ 有 JSON 文件——src 和 evo 的图没有复杂多边形分界线（REE/蛛网图是纯折线，Harker 等是散点图），故对应目录为空。
    ...
    └── batch/
        ...
        └── backgrounds.py  # 批量底图
references/
    architecture-review-20260527.md      # 用户审阅：架构方向与风险清单
    ...
```
...
这意味着：**当你用 pandas 保存为标准格式（Row 1 = 元素名如 SiO2, Col A = 样品名）时，`r1_c2` = 'SiO2' 在 KNOWN_ELEMENTS 中，导致 `transposed = True`**。然后 `_load_transposed` 从 Row 4 开始读样品、只匹配 Row 1 中在 KNOWN_ELEMENTS 的元素（最多 80 列），导致：
- 仅读到 ~16 个 KNOWN_ELEMENTS（主量 + Li/Be/Sc）
- Zr/Nb/Y/V/REE 等微量元素全部丢失
- 如果前几行样品的 A 列值不在 KNOWN_ELEMENTS 中，这些样品行被跳过

**修复方法**：不要直接用 pd.DataFrame.to_excel 保存的"标准格式"喂给 GeochemData。改为用 pandas 转置为真正的标准格式（A1=元素名/样品名，第1列=样品名，第1行=元素名），再保��：

```python
# 原始为转置格式（行=元素，列=样品）
orig = pd.read_excel("input.xlsx")
# 转置为：行=样品，列=元素
df = orig.set_index('Sample').T.reset_index()
df = df.rename(columns={'index': 'Sample'})
# 保存为标准格式，让 GeochemData 走标准路径
# 这样 r1_c2 = 样品名（不在 KNOWN_ELEMENTS 中）→ 正常标准格式
df.to_excel('output_standard.xlsx', index=False)
# GeochemData('output_standard.xlsx') 现在正确读取 89 元素 × 10 样品
```

或用原始文件——如果原始文件是"转置格式（行=样品，列=元素）"，GeochemData 也能正确识别。

**验证方法：** 读取后检查 `gd.all_labels` 列数是否等于预期元素总数。如果只有 ~16 个而你的数据有 89 个元素，说明格式被误判了。
...
元素齐全时的预期结果（见 `quick_validate.py` 的 [14] 元素依赖完整性扫描）：
  - 42 张出图成功，7 张跳过（仅因人造数据缺 Ti/Pb/MnO/P2O5）
...
| `Ti` vs `TiO₂` 不一致导致图被跳过 | 6张图（SRC-02蛛网图、TEC-03 Ti-Zr-Y、TEC-04 四联、TEC-05 Ti-V、TEC-14 Pearce1982、SRC-12 Ti/Yb-Nb/Yb）在 registry 的 `needed` 中写了元素 `Ti`，但 `KNOWN_ELEMENTS` 中只有 `TiO₂`。实际数据都是测 TiO₂ 而非 Ti 单质。若用户数据含 TiO₂ 但缺 Ti，手动将这些图的 needed 中的 `Ti` 改为 `TiO₂` 即可跳过识别——注意图函数内部用的是 `gd.get('Ti')`，所以改 needed 只是让推荐调度允许它们，函数内部仍可能画不出正确图。根本修复：用 `gd.get('TiO2')` 取 Ti 值（TiO₂ × 0.599 = Ti），或让用户数据中同时有 TiO₂ 和 Ti。 |
| 代码实际使用元素 vs 注册表 needed 不一致 | `quick_validate.py --quick` 的元素依赖完整性扫描会检测到此类不一致。当前已知 2 处：`plot_shand` 代码多读 Nb/TiO₂/Y/Zr（着色用），`plot_jensen` 代码多读 CaO/K₂O/MnO/Na₂O（阳离子转换用）。不影响出图，但应留意。 |
...
2. **新增图件**：写绘图函数 + 在 `DIAGRAM_REGISTRY` 加记录 + 在 `_SHORT_TO_LONG` 映射表中加条目（`igneous_wr/report/style.py` 顶部），无需修改 AI 行为。
3. **新增边界坐标**：写入 `igneous_wr/boundaries/{category}/{name}.json` 文件。绘图函数内用 `from igneous_wr.boundaries.core import load_boundary` 导入后调用 `load_boundary(category, name)`。
...
- ✅ **输出文件名必须带注册表编号前缀**：每个绘图函数内部调用 `_style.save_fig(fig, '短名.png', out_dir)`。实际保存时 `save_fig` 通过 `_SHORT_TO_LONG` 映射表自动补编号。**新增图件时必须在 `igneous_wr/report/style.py` 顶部的 `_SHORT_TO_LONG` 中添加映射条目。** 如果遗漏，输出文件名为 `ShortName.png` 而非 `CLS-NN_ShortName.png`。验证方法：查看输出目录中文件名是否以 CLS/SRC/EVO/TEC 开头。`_SHORT_TO_LONG` 与 `FILENAME_MAP`（在 `registry.py` 中）必须保持同步——前者控制实际保存的文件名，后者控制注册表记录。新增图时两处都要加。
- ✅ **新增图件时 source_ref key 要同步 refs.json**：注册表的 `source_ref` 字段现在是 key（如 `"lebas1992"`）而非字符串。新增图时：
  ...
  `_SHORT_TO_LONG`（style.py）、`FILENAME_MAP`（registry.py）、`refs.json`、`registry.py::DiagramSpec.source_ref` — 新增一张图**最多需要同步 4 处**。
...
- ✅ **边界数据不硬编码（SVG 提取的数据存 JSON/YAML，函数只加载不硬编码）**：改坐标时不需深入 1000+ 行的函数文件去搜 `np.array([...])`。用户明确抱怨 `_classification.py` 1423 行太长、"不敢改"。**例外**：用户亲自逐点校准过的定稿版本（如 WF1977 v11）可直接硬编码。
...
- **MORB-OIB 阵列应为灰色半透明带状区域**（由上下两条虚线界定），而非单根红色/黑色曲线。这是 Pearce (2008) 和 Pearce (2014) 原文的核心设计——样品在带内=非俯冲带，偏离向上=SSZ
- **参考点用星号标注**：N-MORB(蓝)、E-MORB(绿)、OIB(橙)，颜色不同便于区分
- **俯冲方向用箭头标注**（SZ 箭头），从阵列内指向高 Th/Yb 方向
- **弧区参考**：Oceanic Arcs 和 Continental Arcs 可用淡色半透明区域表示，帮助判断偏移程度
- **1:1 线** (Th/Yb=Nb/Yb) 作为强烈 Th 富集的参考，即地壳混染/俯冲富集的指示
- 详见 `references/pearce-thyb-nbyb-spec.md`
...
- **重复注册修复经验**：CLS-06（`_classification.py`中的`plot_co_th`）和CLS-25（`_tectonic.py`中的`plot_hastie`）是同一物理图件（Hastie 2007 Co-Th）的不同实现版本，都含有错误。修复方案：保留CLS-06并用文献精确坐标重写，删除CLS-25（包括registry记录、style.py映射、模块import）。删除重复图件时，需检查5处：1) registry.py的DiagramSpec；2) registry.py的import；3) style.py的_SHORT_TO_LONG映射；4) igneous_wr_core.py的import；5) 被删除的绘图函数所在文件。任意一处遗漏都会导致import错误。删除后quick_validate总张数减少1。
...
| 图号 | 图名 | 日期 | 修正内容 | 参考文件 |
|------|------|------|---------|---------|
| SRC-03 | Pearce Th/Yb-Nb/Yb | 2026-06-25 ✅ | 阵列从幂函数(0.03^0.85/0.06^0.90/0.15^0.95)改为 Th/Nb=0.04/0.14 两条斜率为1的对角线；参考点按 S&M89 ppm 严格计算 N-MORB(0.98,0.051)/E-MORB(4.30,0.311)/OIB(26.67,2.222)；新增 UCC 端元(5.45,4.77,R&G2003) | `references/pearce-thyb-nbyb-spec.md` |
...
| CLS-10 | Mullen TiO₂-10×MnO-10×P₂O₅ | 2026-05-29 | 全量重写：6色区(OIT/IAT/MORB/CAB/OIA/BON)替代原4大区；精确端点坐标(A~J, 含F=26.87/28.36/44.78)；实线L1+L2虚线L3+L4区分；新增BON标签 | |
