...
### 3. 图解目录（71 种，含 38 个绘图函数在 4 个模块 + 33 张 XY 二元图在 _xy_diagrams.py）
...
#### XY 二元图（33 张，2026-05-26 完成，位于 `_xy_diagrams.py`）
...
| **Pearce 系列** | Pearce1996 Th/Yb–Nb/Yb | Th, Nb, Yb | 基性 |
| | PearceNorry Zr/Y–Zr | Zr, Y | 基性 |
...
依赖（自动安装）：`matplotlib`, `numpy`, `openpyxl`, `Pillow`
...
├── quick_validate.py       # 验证脚本
        └── whole_rock/
            ├── diagrams/
            │   ├── _xy_diagrams.py      # X-Y 专题图（33张，2026-05-26完成）
...
- ⚠️ **从 SVG 提取的坐标已确认在 `/tmp/extracted_paths.json` 中可用**，像素→三元百分比转换用 `_pixel_to_ternary(px, py)` 公式，已验证通过。不再需要通过 pycache 恢复 _ternary.py 的旧版本。
