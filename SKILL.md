...
**遗留 shim 已清理（2026-06-05）**：`scripts/` 根目录下的 4 个 `sys.modules` 门面文件（`_style.py`, `_chem.py`, `_ternary.py`, `_normalize.py`）已删除。包内所有 import 已改为直接引用 `igneous_wr.report.style` / `igneous_wr.core.chem` 等完整包路径。`scripts/_mullen_swap.py` 也已删除。
