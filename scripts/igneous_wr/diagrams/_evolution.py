...
def plot_miyashiro(gd, out_dir=None, save=True, ax=None):
    ...
    for ann in bd.get('annotations', []):
        ax.text(ann['x'], ann['y'], ann['text'], fontsize=_style.base_fs(ax, scale=ann.get('fontsize', 10) / 10.0), fontstyle=ann.get('fontstyle', 'italic'), ha=ann.get('ha', 'center'), va=ann.get('va', 'center'), color=ann['color'], fontproperties=_style.times_prop)
