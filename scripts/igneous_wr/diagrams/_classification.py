...
def plot_co_th(gd, out_dir=None, save=True):
    ...
    # 系列标注
    ax.text(68, 0.1, 'Tholeiite\nSeries', fontsize=10 * _style.base_fs(ax), fontweight='bold',
    ...
    ax.text(68, 60, 'High-K calc-alkaline\nand Shoshonite Series',
            fontsize=10 * _style.base_fs(ax), fontweight='bold', color=_style.TEXT_COLOR_LABEL, ha='left', va='center')

    # 岩性标注（vs GCDkit adj=0.5 → va='center'）
    ax.text(50, 0.02, 'B', fontsize=10 * _style.base_fs(ax), fontweight='bold', color=_style.TEXT_COLOR_LABEL,
            ha='center', va='center')
    ax.text(30, 0.02, 'BA/A', fontsize=10 * _style.base_fs(ax), fontweight='bold', color=_style.TEXT_COLOR_LABEL,
            ha='center', va='center')
    ax.text(10, 0.02, 'D/R*', fontsize=10 * _style.base_fs(ax), fontweight='bold', color=_style.TEXT_COLOR_LABEL,
            ha='center', va='center')
...
def plot_tas(gd, out_dir=None, save=True, ax=None):
    ...
    for label, (tx, ty) in _text_only.items():
        ax.text(tx, ty, label, ha='center', va='center', fontsize=8 * _style.base_fs(ax), fontweight='normal',
                color=_style.LINE_COLOR_SECONDARY, style='italic', zorder=5)
    for name, poly in _TAS_FIELDS.items():
        cx = sum((p[0] for p in poly)) / len(poly)
        cy = sum((p[1] for p in poly)) / len(poly)
        label = _TAS_LABELS.get(name, name)
        ax.text(cx, cy, label, ha='center', va='center', fontsize=8.5 * _style.base_fs(ax), fontweight='bold', color=_style.TEXT_COLOR_LABEL, zorder=5)
...
def plot_k2o_sio2(gd, out_dir=None, save=True, ax=None):
    ...
    ax.text(80, 1.4, 'Low-K\nTholeiitic', fontsize=10 * _style.base_fs(ax), ha='right', va='center', color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=4)
    ax.text(80, 3.7, 'Medium-K\nCalc-alkaline', fontsize=10 * _style.base_fs(ax), ha='right', va='center', color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=4)
    ax.text(80, 5.7, 'High-K\nCalc-alkaline', fontsize=10 * _style.base_fs(ax), ha='right', va='center', color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=4)
    ax.text(44, 7.4, 'Shoshonitic', fontsize=10 * _style.base_fs(ax), ha='left', va='center', color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=4)
...
def plot_k2o_sio2_peccerillo(gd, out_dir=None, save=True):
    ...
    ax.text(77, 0.7, 'Tholeiite Series', fontsize=10 * _style.base_fs(ax), ha='right', va='center',
            color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=4)
    ax.text(77, 2.4, 'Calc-alkaline\nSeries', fontsize=10 * _style.base_fs(ax), ha='right', va='center',
            color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=4)
    ax.text(77, 4, 'High-K calc-alkaline\nSeries', fontsize=10 * _style.base_fs(ax), ha='right', va='center',
            color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=4)
    ax.text(47, 4.5, 'Shoshonite Series', fontsize=10 * _style.base_fs(ax), ha='left', va='center',
            color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=4)
    _style.scatter_samples(ax, sio2, k2o, labels, groups=gd.groups)
    ax.set_xlim(45, 78)
    ax.set_ylim(0, 7)
...
def plot_afm(gd, out_dir=None, save=True):
    ...
    ax.text(0.5, 0.53, 'Tholeiite Series', fontsize=11 * _style.base_fs(ax), ha='center', va='center', color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=6)
    ax.text(0.5, 0.1, 'Calc-Alkaline Series', fontsize=11 * _style.base_fs(ax), ha='center', va='center', color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=6)
...
def plot_winchester_floyd(gd, out_dir=None, save=True):
    ...
    for label in _WF_LABELS:
        ...
        ax.text(rx, ry, text, fontsize=9.5 * _style.base_fs(ax), color=_style.TEXT_COLOR_LABEL, ha='center', va='center', fontweight='normal', zorder=3)
...
def plot_mullen(gd, out_dir=None, save=True):
    ...
    ax.text(lbl_oit[0][0], lbl_oit[1][0], 'OIT', fontsize=11 * _style.base_fs(ax), ha='center', va='center', color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=5)
    lbl_iat = ternary_to_xy(np.array([34.6]), np.array([50.7]), np.array([14.7]))
    ax.text(lbl_iat[0][0], lbl_iat[1][0], 'IAT', fontsize=11 * _style.base_fs(ax), ha='center', va='center', color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=5)
    lbl_morb = ternary_to_xy(np.array([46.2]), np.array([33.9]), np.array([19.9]))
    ax.text(lbl_morb[0][0], lbl_morb[1][0], 'MORB', fontsize=11 * _style.base_fs(ax), ha='center', va='center', color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=5)
    lbl_cab = ternary_to_xy(np.array([11.5]), np.array([49.2]), np.array([39.2]))
    ax.text(lbl_cab[0][0], lbl_cab[1][0], 'CAB', fontsize=11 * _style.base_fs(ax), ha='center', va='center', color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=5)
    lbl_oia = ternary_to_xy(np.array([25.4]), np.array([10.3]), np.array([64.3]))
    ax.text(lbl_oia[0][0], lbl_oia[1][0], 'OIA', fontsize=11 * _style.base_fs(ax), ha='center', va='center', color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=5)
    lbl_bon = ternary_to_xy(np.array([17.3]), np.array([71.3]), np.array([11.3]))
    ax.text(lbl_bon[0][0], lbl_bon[1][0], 'Bon', fontsize=11 * _style.base_fs(ax), ha='center', va='center', color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=5)
...
def plot_tasmiddlemostplut(gd, out_dir=None, save=True):
    ...
    for name, poly in _MM_FIELDS.items():
        ...
        ax.text(cx, cy, label, ha='center', va='center', fontsize=8.5 * _style.base_fs(ax), fontweight='bold', color=_style.TEXT_COLOR_LABEL, zorder=6)
    _style.scatter_samples(ax, sio2, alk, labels, groups=gd.groups)
    ax.set_xlim(34, 90)
    ax.set_ylim(0, 19)
    ax.set_xticks(range(35, 95, 5))
...
def plot_frost_fenr(gd, out_dir=None, save=True):
    ...
    ax.text(60, 0.9, 'ferroan', fontsize=10 * _style.base_fs(ax), ha='center', va='center',
            color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=5)
    ax.text(60, 0.6, 'magnesian', fontsize=10 * _style.base_fs(ax), ha='center', va='center',
            color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=5)
...
def plot_frost_mali(gd, out_dir=None, save=True):
    ...
    ax.text(56, 7.7, 'alkalic', fontsize=10 * _style.base_fs(ax), ha='center', va='center',
            color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=5)
    ax.text(61, 3.0, 'alkali-calcic', fontsize=10 * _style.base_fs(ax), ha='center', va='center',
            color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=5, rotation=25)
    ax.text(61, 0.5, 'calc-alkalic', fontsize=10 * _style.base_fs(ax), ha='center', va='center',
            color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=5, rotation=25)
    ax.text(65, -2.5, 'calcic', fontsize=10 * _style.base_fs(ax), ha='center', va='center',
            color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=5)
...
def plot_frost_asi_ank(gd, out_dir=None, save=True):
    ...
    ax.text(0.55, 3.0, 'metaluminous', fontsize=10 * _style.base_fs(ax), ha='left', va='center',
            color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=3)
    ax.text(1.85, 3.0, 'peraluminous', fontsize=10 * _style.base_fs(ax), ha='right', va='center',
            color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=3)
    ax.text(0.55, 0.8, 'peralkaline', fontsize=10 * _style.base_fs(ax), ha='left', va='center',
            color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=3)
...
def plot_pearce1996(gd, out_dir=None, save=True):
    ...
    for txt, x, y, rot, ha in text_cfgs:
        ax.text(x, y, txt, fontsize=10 * _style.base_fs(ax), ha=ha, va='center', color=_style.TEXT_COLOR_LABEL, fontweight='bold', rotation=rot, zorder=5)
...
def plot_shand_acnk_ank(gd, out_dir=None, save=True, ax=None):
    ...
    # 分区标注
    ax.text(0.55, 3.5, 'metaluminous', fontsize=10 * _style.base_fs(ax), ha='left', va='center',
            color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=3)
    ax.text(2.5, 3.5, 'peraluminous', fontsize=10 * _style.base_fs(ax), ha='right', va='center',
            color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=3)
    ax.text(0.55, 0.7, 'peralkaline', fontsize=10 * _style.base_fs(ax), ha='left', va='center',
            color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=3)
...
def plot_whalen_ga_al(gd, out_dir=None, save=True, ax=None):
    ...
    ax.text(2.6, 0.6, '2.6', fontsize=9 * _style.base_fs(ax), ha='center', va='top',
            color=_style.TEXT_COLOR_LABEL, zorder=3)
    ax.text(1.0, 8, 'I- & S- & M-type', fontsize=10 * _style.base_fs(ax), ha='center', va='center',
            color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=3)
    ax.text(10, 8, 'A-type', fontsize=10 * _style.base_fs(ax), ha='center', va='center',
            color=_style.TEXT_COLOR_LABEL, fontweight='bold', zorder=3)
