...
def plot_pearce_2008(gd, out_dir=None, save=True):
    ...
    for ann in bd.get('annotations', []):
        ax.text(ann['x'], ann['y'], ann['text'], fontsize=ann.get('fontsize', 10), color=ann.get('color', 'gray'), rotation=ann.get('rotation', 0), ha=ann.get('ha', 'left'), va=ann.get('va', 'bottom'), rotation_mode='anchor', zorder=5)
