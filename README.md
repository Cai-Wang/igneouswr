...
IgneousWR 是一个全岩地球化学批量成图工具。使用 Excel 数据文件，自动生成 19 张校正后的图件（分类图解、源区性质、岩浆演化、构造环境判别）并输出一份自包含的 HTML 报告，有很多功能不成熟。

面向岩石地球化学研究者，无需 GUI 操作，让LLM工具自动化出图。

只负责投图的业务逻辑，不进行视觉效果的工作。
